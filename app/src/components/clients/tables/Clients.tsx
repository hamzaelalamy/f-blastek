...
import {
    actGetClients,
    actDeleteClients,
} from "../../../slices/clients/ClientsSlice";
...
export default function Clients() {
    ...
    const handleDelete = (clienId: React.FormEvent<HTMLButtonElement>) => {
        const confirmDelete = window.confirm("Are you sure?");
        if (confirmDelete) {
            toast("Client deleted successfully", { type: "success" });
            dispatch(actDeleteClients(clienId))
            .then(() => { console.log("Client deleted successfully") })
            .catch(() => { console.log("Error deleting client") });
        } else {
            ...
            <td className="p-4 border-b border-blue-gray-50">
              <div className="w-max">
                <div
                  ...
                  style={{ opacity: 1 }}
                >
                  <span className="">{String(record.backgroundCheckCompleted)}</span>
                </div>
              </div>
            </td>
          ) : (
